...
public class DevConsoleDatabaseHelper extends SQLiteOpenHelper {

    private static final String DB_NAME    = "dev_console.db";
    ...
    private static DevConsoleDatabaseHelper sInstance;

    public static synchronized DevConsoleDatabaseHelper getInstance(Context ctx) {
        if (sInstance == null) {
            sInstance = new DevConsoleDatabaseHelper(ctx.getApplicationContext());
        }
        return sInstance;
    }

    private DevConsoleDatabaseHelper(Context context) {
    ...
    public void saveCollection(SavedLogCollection collection) {
        ...
        try {
            ...
            for (DevConsoleLog log : collection.logs) {
    ...
    public List<DevConsoleLog> getLogsForCollection(String collectionId) {
        List<DevConsoleLog> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        ...
        while (c.moveToNext()) {
            ...
            result.add(new DevConsoleLog(id, level, message, source, timestamp, metadata));
        }
        c.close();
        return result;
    }
    ...
    public static class SavedLogCollection {
        ...
        public final List<DevConsoleLog> logs;
        public final String             savedAt;
        public final int                totalEntries;

        public SavedLogCollection(String id, String name, List<DevConsoleLog> logs,
